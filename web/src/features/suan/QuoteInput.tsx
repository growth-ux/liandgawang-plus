  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl border border-line bg-panel/70 p-5">
        <h3 className="mb-3 text-sm font-semibold">粘贴报价或描述需要比较的方案</h3>
        <textarea
          value={sourceText}
          onChange={(e) => onSourceTextChange(e.target.value)}
          rows={5}
          placeholder={EXAMPLE_TEXT}
          className="w-full resize-none rounded-xl border border-line bg-rice-deep px-4 py-3 text-sm text-ink placeholder:text-ink-soft/50 outline-none focus:border-violet-400"
        />
      ...
      {/* 示例提示 */}
      <div className="rounded-2xl border border-dashed border-line bg-panel/40 px-5 py-4">
        <p className="text-xs text-ink-soft">
          支持格式：自然语言报价、微信消息摘要、合同关键条款。可一次包含多条报价。
        </p>
        <p className="mt-1 text-[11px] text-ink-soft/60">
          示例：{EXAMPLE_TEXT.slice(0, 60)}…
        </p>
      </div>
