...
export default function DecisionTrace({ mission }: DecisionTraceProps) {
  ...
  for (const conflict of mission.conflicts) {
    items.push({ time: mission.updated_at ?? "", actor: "粮掌柜", text: `发现冲突：${conflict.title}` });
  }
  ...
  if (mission.action_tasks.length > 0) {
    items.push({ time: mission.updated_at ?? "", actor: "粮掌柜", text: `生成 ${mission.action_tasks.length} 项行动任务` });
  }
        ...
        {items.map((item, index) => (
          <li key={index} className="flex items-start gap-3 text-sm">
            ...
            <div className="min-w-0">
              ...
              {item.time && (
                <span className="ml-2 text-xs text-ink-soft/70">{item.time.replace("T", " ").slice(11, 19)}</span>
              )}
            </div>
          </li>
        ))}
