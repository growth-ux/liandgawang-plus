...
export default function MissionCockpit({ mission, onMissionChange, onBack }: MissionCockpitProps) {
  ...
  return (
    <div className="zg-cockpit mx-auto w-full max-w-[1440px] px-6 py-6">
      ...
      {mission.status === "completed" && <section className="my-4 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-tech/30 bg-tech/5 px-5 py-4"><div><h2 className="text-sm font-medium text-tech">方案已确认，继续同一笔采购</h2><p className="mt-1 text-xs leading-6 text-ink-soft">带入本任务需求、已选粮源、运输与成本结果，继续准入核验、下单交付、到厂核算和履约复盘。</p>{purchaseError && <p role="alert" className="mt-2 text-xs text-amber-300">{purchaseError}</p>}</div><button type="button" className="rounded-lg bg-brand px-5 py-3 text-sm text-slate-950" onClick={continuePurchase}>继续办理采购 →</button></section>}
