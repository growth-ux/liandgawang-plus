...
export default function TopNav() {
  return (
    <header className="sticky top-0 z-40 h-16 border-b border-line bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-full max-w-[1440px] items-center justify-between px-6">
        {/* 品牌标识，点击返回首页 */}
        <NavLink to="/" className="flex items-center gap-2.5">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-brand to-brand-deep text-lg font-semibold text-white shadow-sm">
            粮
          </span>
              ...
              className={({ isActive }) =>
                `rounded-full px-5 py-2 text-sm transition-colors ${
                  isActive
                    ...
                    : "text-ink-soft hover:bg-rice hover:text-ink"
                }`
              }
            >
              {item.label}
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            aria-label="消息"
            className="relative flex h-9 w-9 items-center justify-center rounded-full text-ink-soft transition-colors hover:bg-rice hover:text-ink"
          >
